...
#define RX_PAYLOAD_LENGTH 200
LoRaClass* lora;
long frequency = mgos_sys_config_get_lora_frequency();;
...
static void button_on(int pin, void *arg)
{
  dest_id = mgos_sys_config_get_app_dest_id();
  char tx_buf[TX_PAYLOAD_LENGTH] = {0};
  createCmd(tx_buf, (int)BUTTON_ON, dest_id,1);
...
static void button_off(int pin, void *arg)
{
  dest_id = mgos_sys_config_get_app_dest_id();
  char tx_buf[TX_PAYLOAD_LENGTH] = {0};
  createCmd(tx_buf, (int)BUTTON_OFF, dest_id,0);
...
bool ldr_status(void)
{
  ...
  /* Reference Voltage 200mv(As per tested conditions) */
  if (mgos_adc_read_voltage(ldr_pin) > mgos_sys_config_get_sys_esp32_adc_vref())
    return true;
  else
    return false;
}
       ...
       if(strcmp(received_dest_id,dest_id) == 0)
       {
        ...
        case BUTTON_OFF:
        {
          ...
          //char tx_buf[data_len+1] = {0};
            createCmd(tx_buf, (int)ACKNOWLEDGEMENT_OFF, received_dest_id,0);
            motor_state = 0;
            mgos_send_lora(lora,tx_buf);
        ...
        case BUTTON_ON:
        {
          ...
          //char tx_buf[data_len+1] = {0};
            motor_state = 1;
          createCmd(tx_buf, (int)ACKNOWLEDGEMENT_ON,received_dest_id,1);
...
enum mgos_app_init_result mgos_app_init(void) {
 ...
 if (!mgos_sys_config_get_app_receiver())
 {
  ...
  /* Pin number 36 for LDR at receiver*/
